...
int g_direction = 1;
int vitesse = 300;
float vit_motg_Origin = 0.55;
float vit_motd_Origin = 0.55;
...
void Avancer(int pulse)
{
    ...
    while(dist_reel_totG < pulse - derniereValeurLuGPulse)
    {
        ...
        if (vit_motg < vit_motg_Origin && accel == true)
            vit_motg += 0.03;
        else
        {
            accel = false;
        ...
        if (vit_motd < vit_motd_Origin && accel == true)
            vit_motd += 0.03;
        else
        {
            accel = false;
        ...
        //decceleration
        if(pulse - dist_reel_totG < 6400 & vit_motd > 0.1 && vit_motg > 0.1)
        {
            vit_motg -= 0.028;
            vit_motd -= 0.028;
...
{    
    ...
    vit_motd = g_direction * -0.14;
    vit_motg = g_direction * 0.14;
